...
export default function SideBSubmission() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        sessionStorage.setItem('sideBData', JSON.stringify(result.data));
        
        // Check if Side A submitted
        const sideAData = sessionStorage.getItem('sideAData');
        
        if (sideAData) {
          // Both sides submitted 
          alert('Side B submitted successfully! Both sides have now submitted. Proceeding to arguments...');
          window.location.href = '/case/arguments';
        } else {
          // For Side A
          alert('Side B submitted successfully! Please submit Side A arguments now.');
          const proceed = confirm('Would you like to submit for Side A now?');
          if (proceed) {
            window.location.href = '/case/side-a';
          } else {
  ...
  const handleSaveDraft = async () => {
    try {
      ...
      localStorage.setItem('case-side-b-draft', JSON.stringify(draftData));
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-4">
              <Link href="/case/side-a" className="text-sm text-slate-600 hover:text-blue-600 transition-colors">
                ← Side A Submission
              </Link>
              <div className="hidden sm:flex items-center gap-2 bg-blue-50 px-4 py-2 rounded-lg">
                <div className="w-2 h-2 bg-blue-600 rounded-full animate-pulse"></div>
                <span className="text-sm font-medium text-blue-900">Side B Submission</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h1 className="text-3xl sm:text-4xl font-bold text-slate-900">Case Submission for Side B</h1>
              <p className="text-slate-600 mt-1">Defendant / Respondent</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6 sm:p-8 mb-6 hover:shadow-xl transition-shadow">
          ...
          <div className="mb-6">
            <label className="block text-sm font-semibold text-slate-900 mb-2">
              Primary Argument for Side B <span className="text-red-500">*</span>
            </label>
            <textarea
              value={primaryArgument}
              onChange={(e) => setPrimaryArgument(e.target.value)}
              placeholder="Summarize your core legal argument for Side B, highlighting key facts and legal precedents..."
              rows={6}
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-900 mb-2">
              Response to Side A's Claims
            </label>
            <textarea
              value={responseToA}
              onChange={(e) => setResponseToA(e.target.value)}
              placeholder="Address and counter the claims made by Side A, presenting your rebuttals and alternative interpretations..."
              rows={6}
              className="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow resize-none text-slate-900 placeholder:text-slate-400"
              disabled={isSubmitting}
            />
